...
class TestCarController:
    ...
    async def test_get_all_cars(self, mock_get_all_cars):
        ...
        mock_get_all_cars.return_value = [Car(id="1", name="Toyota"), Car(id="2", name="Honda")]
        
        result = await get_all_cars()
        
        assert len(result) == 2
        assert result[0].name == "Toyota"
    ...
    async def test_get_car_by_id_when_exists(self, mock_get_car_by_id):
        ...
        mock_get_car_by_id.return_value = Car(id="1", name="Toyota")
    ...
    async def test_create_car(self, mock_create_car):
        ...
        car_data = CreateCarDto(name="Toyota")
        mock_create_car.return_value = Car(id="1", name="Toyota")
    ...
    async def test_update_car_when_exists(self, mock_update_car):
        ...
        car_data = UpdateCarDto(name="Updated Toyota")
        mock_update_car.return_value = Car(id="1", name="Updated Toyota")
        
        result = await update_car("1", car_data)
        
        assert result.id == "1"
        assert result.name == "Updated Toyota"
    ...
    async def test_update_car_when_not_exists(self, mock_update_car):
        ...
        with pytest.raises(HTTPException) as exc_info:
            await update_car("999", UpdateCarDto(name="Nonexistent"))
