...
class TestOrderService:
    ...
    async def test_create_order_should_add_new_order(self):
        # Arrange
        new_order_data = CreateOrderDto(userId="3", items=[OrderItem(productId="4", quantity=1, price=100.0)], total=None, status=None)
    ...
    async def test_update_order_should_modify_existing_order(self):
        # Arrange
        update_data = UpdateOrderDto(items=[OrderItem(productId="1", quantity=3, price=3500.0)], total=10500.0)
    ...
    async def test_update_order_should_return_none_when_order_does_not_exist(self):
        # Arrange
        update_data = UpdateOrderDto(total=5000.0)
    ...
    async def test_delete_order_should_return_false_when_order_does_not_exist(self):
        # Act
        result = await delete_order("999")

        # Assert
        assert result is False
        assert len(orders) == 3  # Ensure orders remain unchanged